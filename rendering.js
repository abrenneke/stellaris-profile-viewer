...
export function renderRow(data, parentName = "") {
  ...
  if (isRoot) {
    ...
    rootPctCol.innerText = `${(
      data.root_pct * 100
    ).toLocaleString()}% of parent`;
  } else {
    const rootText = `${(data.root_pct * 100).toLocaleString()}%`;
    rootPctCol.innerText =
      rootText.length > 14 ? rootText.substring(0, 14) + "..." : rootText;
  }
  ...
  totalTimeCol.innerText = isRoot
    ? `${formatTime(data.total_time)} total`
  ...
  const averageText = data.hits
    ? `avg ${formatTime(data.total_time / data.hits)}`
    : "";
...
function toggleRow(row) {
  ...
  if (childContainer) {
    ...
    const children = Array.from(childContainer.children).filter((child) =>
