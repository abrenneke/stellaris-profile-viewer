...
export function renderRow(data, parentName = '') {
    ...
    hitsCol.innerText = isRoot ? `${data.hits.toLocaleString()} hits` : data.hits.toLocaleString(undefined, { maximumFractionDigits: 0 });
...
function toggleRow(row) {
    ...
    if (childContainer) {
        ...
        const children = Array.from(childContainer.children)
            .filter(child => child.classList.contains('table-row'));
